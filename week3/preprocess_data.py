...
def preprocess_data(file_path, data_dir):
    ...
    for pha in phase:
        df = pd.read_csv(os.path.join(file_path, f'{pha}.csv'))
        ...
        data[pha]['articles'] = [preprocess_text(article) for article in tqdm(articles)]
        data[pha]['highlights'] = [preprocess_text(highlight) for highlight in tqdm(highlights)]
