...
def get_all_sentences(dataset, lang):
    # translation / en it
    # 로 이루어져 있는 dataset에서 문장을 꺼내옴
    for item in dataset:
        yield item["translation"][lang]

def get_or_build_tokenizer(config, dataset, lang):
    # config["tokenizer_file"] = "../tokenizer_{0}.json"
    # eng의 경우: tokenizer_en.json으로 저장되게 됨 (lang = en)
    ...
    if not Path.exists(tokenizer_path):
        ...
        # unkwon, padding, start of sentence, end of sentence를 추가하고 최소 2번 나온 단어를 tokenization
...
def get_or_build_tokenizer_non_space(config, dataset, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        # 텍스트로 토큰을 나누기 전 단어와 서브워드 단위로 분류하게 됨
